...
import { FLOAT_NEGATIVE_REG_EXP } from '../utils/functions';
...
@Component({
  ...
  selector: 'float-cell',
  templateUrl: './float-negative-editor.component.html',
  styleUrls: ['./float-negative-editor.component.scss'],
})
// tslint:disable-next-line: component-class-suffix
export class FloatNegativeEditor implements ICellEditorAngularComp, AfterViewInit {
  private params: any;
  public value: number;
  ...
  isCancelAfterEnd(): boolean {
    return (this.value > 1000000 || (this.value && !this.isAValidFloatingNumber(this.value.toString())));
  }
  ...
  private alreadyHasDecimalPoint(value: string): boolean {
  ...
  private isAValidFloatingNumber(valueToTest: string): boolean {
    return FLOAT_NEGATIVE_REG_EXP.test(valueToTest);
  }
  ...
  private isKeyPressedNumeric(event: any): boolean {
    ...
    const currentValue = this.input.element.nativeElement.value;
    return this.isAValidFloatingNumber(initialChar + currentValue + charStr);
  }
